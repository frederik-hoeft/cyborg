...
- **Collections** — Properties typed as supported enumerable shapes are rewritten and validated element-by-element when their element type requires work. `CollectionTypeInspector` produces one `CollectionShape` that records element type, default-state semantics, count capability, and rewrite materialization for arrays, `List<T>`, `ImmutableArray<T>`, supported collection interfaces, and constructible concrete collections; `string` is explicitly excluded and remains a scalar despite implementing `IEnumerable<char>`. `CollectionCodeGeneration` is the single emission boundary for collection access and materialization. It supplies the guard and usable value expression consumed by validation, interpolation, defaults, and description generation, so null references, nullable value types, and default `ImmutableArray<T>` values have identical absence semantics in every phase. The same abstraction supplies safe constant-time count expressions to length validation and prevents default immutable arrays from being counted or enumerated.
