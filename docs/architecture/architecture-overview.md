...
This configuration layer is also the selection mechanism for keyed host services. For example, the debugger frontend selection key reads `cyborg.core.debug:frontend`, while the CLI separately decides which concrete frontend implementations to register. Host policy can be expressed as an ordinary low-precedence source: when CLI breakpoint arguments activate debugging, the CLI contributes its console frontend selection through a dictionary-backed source before loading the options file, so explicit configuration still wins. Keeping source composition, option values, and keyed-service registration separate avoids moving host policy into `Cyborg.Core`.
