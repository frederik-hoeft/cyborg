...
3. **Post-execution hooks** run after the worker returns its execution result and receive that result together with the runtime. They observe completed worker execution without changing the result contract. Workers finalize results through `runtime.Exit(...)` before returning, so post-execution hooks currently observe already-finalized artifact publication.

If no pre-execution hook short-circuits the module, `ModuleWorker<TModule>` enforces the final validation result with `EnsureValid()`. Invalid results therefore fail before worker code executes, while valid results proceed to `ExecuteAsync`. This keeps worker implementations isolated from lifecycle infrastructure while allowing cross-cutting services to participate at explicit execution boundaries.
...
Host-level service options are composed separately from workflow environment variables. `IConfigurationBuilder` collects configuration loaders and ignored keys, materializes their sources asynchronously, and finalizes the singleton `IConfiguration` once the host composition is complete. File-backed and dictionary-backed loaders use the same source abstraction, allowing the CLI, tests, and other hosts to provide configuration through different inputs without changing option consumers.

Configuration values are flattened into colon-delimited keys. Values implementing `IDecomposable` contribute nested keys recursively, so a structured options value registered at `cyborg.core.debug` exposes fields such as `cyborg.core.debug:frontend`. When multiple configuration sources provide the same flattened key, later-applied sources replace earlier values; ignored keys are omitted while sources are incorporated. Once finalized, consumers read a stable configuration snapshot through `IConfiguration`.

This configuration layer is also the selection mechanism for keyed host services. For example, the debugger frontend selection key reads `cyborg.core.debug:frontend`, while the CLI separately decides which concrete frontend implementations to register. Keeping source composition, option values, and keyed-service registration separate allows hosts to choose their own defaults and configuration sources without moving host policy into `Cyborg.Core`.
