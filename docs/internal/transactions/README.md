...
The current architecture remains described by [System Architecture](../../architecture/architecture-overview.md), [Source Generators](../../architecture/source-generators.md), and [Module Testing Architecture](../../architecture/module-testing.md). The migration changes several current ownership boundaries. Loaded module references are worker-free, workers are activated immediately before execution, every invocation owns a DI scope and child transaction, independently resolved root runtimes own separate logical-global environments, and the complete runtime environment graph (bindings, named registration, topology, and transient lifetime) resolves through transaction-local participant state. The named-module registry remains the next mutable workflow-state boundary.
