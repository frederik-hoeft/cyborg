...
public sealed class DynamicKeyValuePairJsonConverter(IDynamicValueProviderRegistry registry, IJsonLoaderContextProvider provider) : ExtendedJsonConverter<DynamicKeyValuePair>(provider)
{
    public override DynamicKeyValuePair Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        // key null check is handled by the pre-execution pipeline ([Required] attribute)
        // we only check for null value here, because it is part of the JSON contract that the value must be present for a dynamic key-value pair.
