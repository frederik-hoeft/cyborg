...
internal sealed class RuntimeEnvironmentGraphFork(TransactionalDictionary<RuntimeEnvironmentId, RuntimeEnvironmentNode> nodes, TransactionalDictionary<string, RuntimeEnvironmentId> registrations)
{
    private readonly TransactionalDictionaryFork<RuntimeEnvironmentId, RuntimeEnvironmentNode> _nodes = new(nodes);
    private readonly TransactionalDictionaryFork<string, RuntimeEnvironmentId> _registrations = new(registrations);

    public RuntimeEnvironmentGraphState CreateBranch() => new(_nodes.CreateBranch(), _registrations.CreateBranch());
        ...
        [NotNullWhen(false)] out TransactionConflict? conflict)
    {
        ...
        TransactionalDictionary<RuntimeEnvironmentId, RuntimeEnvironmentNode>[] nodeContributors = [.. contributors.Select(static state => state.Nodes)];
        TransactionalDictionary<string, RuntimeEnvironmentId>[] registrationContributors = [.. contributors.Select(static state => state.Registrations)];

        if (!_nodes.TrySelectChanges(participant, nodeContributors,
                selectLogicalKey: static key => new RuntimeEnvironmentTopologyConflictKey(key),
                conflictStrategy,
            ...
            || !_registrations.TrySelectChanges(
                ...
                selectLogicalKey: static key => new RuntimeEnvironmentRegistrationConflictKey(key),
                conflictStrategy,
    ...
    private HashSet<RuntimeEnvironmentId> DetermineRetainedEnvironmentIds(TransactionalDictionary<RuntimeEnvironmentId, RuntimeEnvironmentNode> nodes, TransactionalDictionary<string, RuntimeEnvironmentId> registrations)
    {
    ...
    private static void ValidateRegistrations(TransactionalDictionary<RuntimeEnvironmentId, RuntimeEnvironmentNode> nodes, TransactionalDictionary<string, RuntimeEnvironmentId> registrations)
    {
