...
/// </remarks>
internal sealed class TransactionalDictionary<TKey, TValue> : IReadOnlyDictionary<TKey, TValue> where TKey : notnull
{
    ...
    private TransactionalDictionary(TransactionalDictionarySnapshot<TKey, TValue> baseline)
        : this(baseline, new Dictionary<TKey, TransactionalDictionaryChange<TValue>>(baseline.Data.KeyComparer), baseline)
    {
    }

    private TransactionalDictionary(TransactionalDictionarySnapshot<TKey, TValue> baseline, Dictionary<TKey, TransactionalDictionaryChange<TValue>> changes, TransactionalDictionarySnapshot<TKey, TValue> effectiveSnapshot)
    {
        Baseline = baseline;
        _changes = changes;
    ...
    internal TransactionalDictionarySnapshot<TKey, TValue> Baseline { get; }
    ...
    public bool TryGetValue(TKey key, [MaybeNullWhen(false)] out TValue value)
    {
        ...
        return Baseline.TryGetValue(key, out value);
    }
        ...
        out TKey conflictKey)
    {
        ...
        Dictionary<TKey, TransactionalDictionaryChange<TValue>> contributorChanges = new(Baseline.Data.KeyComparer);
    ...
    internal TransactionalDictionary<TKey, TValue> PrepareMergeCandidate(TransactionalDictionarySnapshot<TKey, TValue> forkBaseline, IReadOnlyDictionary<TKey, TransactionalDictionaryChange<TValue>> contributorChanges)
    {
        ArgumentNullException.ThrowIfNull(forkBaseline);
        ...
        Dictionary<TKey, TransactionalDictionaryChange<TValue>> candidateChanges = new(_changes, Baseline.Data.KeyComparer);
        ...
        return new TransactionalDictionary<TKey, TValue>(Baseline, candidateChanges, candidateSnapshot);
    }
