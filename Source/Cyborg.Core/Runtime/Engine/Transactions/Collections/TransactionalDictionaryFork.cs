...
internal sealed class TransactionalDictionaryFork<TKey, TValue>(TransactionalDictionary<TKey, TValue> owner) where TKey : notnull
{
    private readonly TransactionalDictionarySnapshot<TKey, TValue> _baseline = owner.Freeze();
    ...
    public bool TrySelectChanges(ITransactionParticipant participant,
        ...
        [NotNullWhen(false)] out TransactionConflict? conflict)
    {
        ...
        Dictionary<TKey, List<ContributorChanges<TValue>>> changes = new(_baseline.Data.KeyComparer);
        for (int contributorIndex = 0; contributorIndex < contributors.Count; ++contributorIndex)
        {
            ...
            {
                if (!changes.TryGetValue(key, out List<ContributorChanges<TValue>>? keyChanges))
                {
                    keyChanges = [];
                    changes.Add(key, keyChanges);
                }
                keyChanges.Add(new ContributorChanges<TValue>(contributorIndex, change));
            }
        }

        selectedChanges = new Dictionary<TKey, TransactionalDictionaryChange<TValue>>(_baseline.Data.KeyComparer);
        foreach ((TKey key, List<ContributorChanges<TValue>> keyChanges) in changes)
        {
    ...
    public TransactionalDictionary<TKey, TValue> PrepareCandidate(IReadOnlyDictionary<TKey, TransactionalDictionaryChange<TValue>> selectedChanges)
    {
        ArgumentNullException.ThrowIfNull(selectedChanges);
        EnsureOwnerUnchanged();
        return owner.PrepareMergeCandidate(_baseline, selectedChanges);
    }

    private void EnsureOwnerUnchanged()
    {
        if (!ReferenceEquals(owner.Freeze(), _baseline))
        {
...
file record struct ContributorChanges<TValue>(int ContributorIndex, TransactionalDictionaryChange<TValue> Change);
