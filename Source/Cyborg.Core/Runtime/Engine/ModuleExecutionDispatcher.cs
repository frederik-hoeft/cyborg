...
internal sealed class ModuleExecutionDispatcher(IRuntimeEnvironmentFactory environmentFactory, ILoggerFactory loggerFactory) : IModuleExecutionDispatcher
{
    ...
    public async Task<IModuleExecutionResult> ExecuteAsync(IModuleWorker module, IModuleRuntime runtime, IRuntimeEnvironment environment, IServiceProvider? serviceProvider, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException)
        {
            ...
            result = new ModuleExecutionResult(module.Module, ModuleExitStatus.Canceled, environmentFactory.CreateEnvironmentLike(environment.Namespace));
        }
        catch (Exception exception)
        {
            _logger.LogModuleUnhandledException(module.ModuleId, environment.Name, exception);
            result = new ModuleExecutionResult(module.Module, ModuleExitStatus.Failed, environmentFactory.CreateEnvironmentLike(environment.Namespace));
        }
    ...
    private async ValueTask RunPostExecutionHooksAsync(string moduleId, IModuleExecutionResult result, IModuleRuntime runtime, IServiceProvider? serviceProvider)
    {
