...
public abstract class ParserBase<TVisitor>(string? name) : ParserBase(name) where TVisitor : class, INodeVisitor
{
    ...
    {
        if (TryParse(input, out ISyntaxNode<TVisitor>? typedNode, out charsConsumed))
        {
    ...
    protected abstract bool TryParse(ReadOnlySpan<char> input, [NotNullWhen(true)] out ISyntaxNode<TVisitor>? syntaxNode, out int charsConsumed);
}
