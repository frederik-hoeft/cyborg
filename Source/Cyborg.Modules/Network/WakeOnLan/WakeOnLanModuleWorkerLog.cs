...
{
    [ZLoggerMessage(LogLevel.Debug, "Checking if '{targetHost}' is already reachable on port {port} with discovery timeout of {discoveryTimeout}")]
    public static partial void LogWolCheckingReachability(this ILogger logger, string targetHost, int port, string discoveryTimeout);
