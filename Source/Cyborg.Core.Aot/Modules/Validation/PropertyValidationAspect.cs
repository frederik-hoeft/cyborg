...
internal abstract class PropertyValidationAspect
{
    ...
    public virtual string? RewriteOverrideResolutionExpression(
        PropertyRewriteContext context,
        string? currentExpression,
        string rootPathExpression)
        => currentExpression;

    public virtual string? RewriteDefaultAssignmentExpression(
        PropertyRewriteContext context,
        string? currentExpression)
        => currentExpression;
    ...
    protected virtual void EmitValidation(
        IndentedStringBuilder builder,
        ModulePropertyModel model)
    {
    }
    ...
    protected static string CreateValidationError(
        ModulePropertyModel model,
        string rule,
        string message) =>
        $"""
    ...
    protected sealed record ModulePropertyModel(
        ...
        string? ExplicitAccessExpression)
    {
        public string AccessExpression =>
            field ??= ExplicitAccessExpression
                ?? $"{ModuleVariable}.{Property.Name}";
    }
}
